...
int json_object_set_new_nocheck(json_t *json, const char *key, json_t *value)
{
    ...
    object = (json_object_t *)json_to_object(json);

    k = (object_key_t *)malloc(sizeof(object_key_t) + strlen(key) + 1);
    if(!k)
        return -1;
...
{
    if(!iter)
        return (const object_key_t *)NULL;
...
json_t *json_array(void)
{
    ...
    array->table = (json_t **)malloc(array->size * sizeof(json_t *));
    if(!array->table) {
        free(array);
        return NULL;
    }
                                ...
                                int copy)
{
    ...
    new_table = (json_t **)malloc(new_size * sizeof(json_t *));
    if(!new_table)
        return NULL;
...
{
    json_integer_t *integer = (json_integer_t *)malloc(sizeof(json_integer_t));
    if(!integer)
        return NULL;
...
{
    json_real_t *real = (json_real_t *)malloc(sizeof(json_real_t));
    if(!real)
        return NULL;
